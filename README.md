...
Скопируйте файл `.env.example` в `.env` и заполните все переменные.
...
и DNS в `ip_tree.md`.  Marzban автоматически будет управлять узлом через
свой REST API.
...
файле `.env.example` присутствуют наиболее важные из них с пояснениями.
